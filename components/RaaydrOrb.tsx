// RAAYDR Halo — adapted from React Bits "Orb" (https://reactbits.dev/backgrounds/orb)
// Original source: https://github.com/DavidHDev/react-bits (MIT + Commons Clause)
// Rendering technique confirmed to match halobase.ai's live WebGL canvas (OGL library,
// GLSL fragment shader, YIQ-space hue rotation, simplex-noise-driven organic swirl).
//
// Changes from the original component:
//   1. `ambientRotation` replaces hover-gated rotation — halobase's ring rotates
//      continuously and slowly regardless of mouse position, not just on hover.
//   2. `backgroundColor` defaults to RAAYDR's locked Canvas token (#F5F2EC).
//   3. Rotation speed is slowed to feel ambient rather than interactive.
//
...
interface RaaydrOrbProps {
  ...
  hue?: number;
  /** How strongly the mouse-proximity ripple distorts the surface. 0 disables it. */
  hoverIntensity?: number;
  /** If true, the ring rotates continuously on its own (halobase behaviour). */
  ambientRotation?: boolean;
  /** Degrees per second of ambient rotation. Keep this low — halobase reads as near-static. */
  ambientRotationSpeed?: number;
  /** Still supported: rotate only while hovered, on top of/instead of ambient rotation. */
...
export default function RaaydrOrb({
  ...
  useEffect(() => {
    ...
    const ambientRotationRadPerSec = (ambientRotationSpeed * Math.PI) / 180;
    ...
    const update = (t: number) => {
      ...
      // Ambient rotation: always-on, slow — this is what halobase actually does.
      if (ambientRotation) {
        currentRot += dt * ambientRotationRadPerSec;
      }
      // Optional extra rotation boost on hover, on top of ambient.
  ...
  }, [hue, hoverIntensity, ambientRotation, ambientRotationSpeed, rotateOnHover, forceHoverState, backgroundColor]);

  // Inline style rather than the original's Tailwind classes — this project
  // has no Tailwind (see AGENTS.md), and without full width/height the
  // container collapses to 0px and the canvas never shows.
