...
// RAAYDR — PageSpectraNoise
// Ported from a purchased Framer marketplace component ("Spectra Noise").
// Framer-only APIs (addPropertyControls, ControlType, useIsStaticRenderer,
...
export interface PageSpectraNoiseProps {
  /** Which RAAYDR audience page this is on — sets the exact colour automatically. */
  audience: RaaydrAudience;
  colorIntensity?: number; // 0-1, how strongly the audience colour overrides the base pattern
...
export default function PageSpectraNoise({
  audience,
  colorIntensity = 0.85,
  noiseIntensity = 0.05,
  scanlineIntensity = 0,
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      gl!.uniform1i(uniforms.uUseCustomColors, 1);
      gl!.uniform3f(uniforms.uPrimaryColor, colors.primary[0], colors.primary[1], colors.primary[2]);
  ...
  if (!webglSupported) {
    return (
      <div style={{ width: '100%', height: '100%', background: '#F5F2EC', ...style }} />
    );
  }
