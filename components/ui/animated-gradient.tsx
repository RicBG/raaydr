...
// Only one preset: RAAYDR. Canvas base, Signal Green as a faint blurred
// accent. Everything else in the original prompt (Aurora/Oceanic/Amber/
// Toxic/Ghost) was a dark neon palette that doesn't belong on this site,
// so it's been removed rather than kept around unused.
const presets: Record<PresetName, PresetParams> = {
  Raaydr: {
    color1: "#F5F2EC", // Canvas
    color2: "#EDE8DE", // Canvas, slightly deeper
    color3: "#3BCE7B", // Signal Green, used only as soft light
    rotation: 20,
    proportion: 22, // small — keep the accent light, not a shape
    scale: 0.5,
    speed: 6, // slow, ambient
    distortion: 10,
    swirl: 30,
    swirlIterations: 6,
    softness: 100, // fully soft edges, no hard boundary
    offset: 0,
    shape: "Edge",
    shapeSize: 60,
  },
...
function GradientFallback({ className, style }: { className?: string; style?: CSSProperties }) {
  // Static CSS fallback if WebGL2 isn't available. Uses the same brand
  // tokens so there's no visible seam if this ever kicks in.
  return (
    <div
      className={className}
      style={{
        ...style,
        background:
          "radial-gradient(120% 120% at 20% 20%, rgba(59,206,123,0.10) 0%, rgba(245,242,236,0) 55%), #F5F2EC",
      }}
    />
  );
}
  ...
  const params = useMemo((): PresetParams => {
    if (config.preset === "custom") {
      return {
        ...
        proportion: config.proportion ?? 22,
        scale: config.scale ?? 0.5,
        speed: config.speed ?? 6,
        distortion: config.distortion ?? 10,
        swirl: config.swirl ?? 30,
        swirlIterations: config.swirlIterations ?? 6,
        softness: config.softness ?? 100,
        offset: config.offset ?? 0,
        shape: config.shape ?? "Edge",
        shapeSize: config.shapeSize ?? 60,
      };
    }
    const preset = presets[config.preset] || presets.Raaydr;
    return {
      ...preset,
